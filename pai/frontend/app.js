...
async function chat() {
    ...
    if (data) {
        ...
        const mocks = [
            "I'm currently in 'Offline Mode'. To enable full AI intelligence, please ensure the Python backend is running. 🚀",
            "Great to see you! As your mentor, I recommend checking your sprint backlog while the AI engine connects.",
            "That's a great point! Let's focus on your top task for today while I re-establish connection to the neural hub."
        ];
        const randomMock = mocks[Math.floor(Math.random() * mocks.length)];
        addBubble('ai', randomMock);
    }
}
