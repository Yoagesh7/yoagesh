...
@tasks_bp.route('/stats/streak', methods=['GET'])
def get_streak():
    return jsonify({"streak": 14})

# Initialize with sample tasks if empty
if not get_all_tasks():
    add_task("Review portfolio design", "2026-04-14", "Make sure the dark mode is consistent.")
    add_task("Deploy AI dashboard", "2026-04-15", "Ensure all API keys are secured.")
    add_task("Refactor agentic loops", "2026-04-16", "Optimize the NVIDIA API calls.")
